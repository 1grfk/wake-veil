...
METADATA
{
  ...
  "tools": [
    ...
    {
      "name": "budget_consume_actual",
      "description": {
        "zh": "按主窗口累计token差值记账：传唤醒前后的 inputTokens/outputTokens，工具内部算差值记入今日账本（真实消耗）。",
        "en": "Record actual usage by delta of cumulative chat tokens."
      },
      "parameters": [
        {
          "name": "behavior_type",
        ...
        },
        {
          "name": "before_input",
          "description": {
            "zh": "唤醒前累计 inputTokens",
            "en": "Cumulative input tokens before"
          },
          "type": "integer",
          "required": true
        },
        {
          "name": "before_output",
          "description": {
            "zh": "唤醒前累计 outputTokens",
            "en": "Cumulative output tokens before"
          },
          "type": "integer",
          "required": true
        },
        {
          "name": "after_input",
          "description": {
            "zh": "唤醒后累计 inputTokens",
            "en": "Cumulative input tokens after"
          },
          "type": "integer",
          "required": true
        },
        {
          "name": "after_output",
          "description": {
            "zh": "唤醒后累计 outputTokens",
            "en": "Cumulative output tokens after"
          },
          "type": "integer",
          "required": true
        },
...
async function budget_consume_actual(params) {
  try {
    const bt = String(params.behavior_type || "").trim();
    if (!VALID_BEHAVIORS.includes(bt)) {
      complete({ success: false, message: "未知行为类型: " + bt });
    ...
    const beforeIn = Math.max(0, parseInt(params.before_input) || 0);
    const beforeOut = Math.max(0, parseInt(params.before_output) || 0);
    const afterIn = Math.max(0, parseInt(params.after_input) || 0);
    const afterOut = Math.max(0, parseInt(params.after_output) || 0);
    const note = String(params.note || "");
    const inTok = Math.max(0, afterIn - beforeIn);
    const outTok = Math.max(0, afterOut - beforeOut);
    const ledger = await loadLedger();
    const day = getToday(ledger);
    const cost = estimateCost(ledger, inTok, outTok);
    const bu = behaviorUsed(day, bt);
    ...
    bu.cost_yuan += cost;
    bu.count += 1;
    pushLog(ledger, { t: nowLocal(), type: "consume", behavior: bt, inTok: inTok, outTok: outTok, cost: Number(cost.toFixed(6)), note: note, actual: true });
    await saveLedger(ledger);
    complete({
      success: true,
      data: {
    ...
    complete({ success: false, message: "budget_consume_actual 失败: " + (e && e.message ? e.message : e) });
  }
}
...
exports.budget_consume_actual = budget_consume_actual;

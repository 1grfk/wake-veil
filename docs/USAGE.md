...
| ③ 工作流 | 外部 workflow | 喊人：每 15 分钟调度，串联各闸门 |
| ④ 预算审核 | `autonomous_budget` | 把关：唤醒要花钱，先查账再放行 |
...
- **工具包**：`autonomous_budget`
  API：`budget_check` / `budget_consume` / `budget_status` / `budget_request_topup` / `budget_topup_approve` / `budget_set_behavior`
- **规则**：默认 2 元 / 8 万 token 每天，行为类型 `proactive_message`（自发唤醒归此类）
- **职责**：唤醒要花钱（调模型），先查账（allow=true）才放行，预算不足静默跳过、可申请追加
- **用途**：防止自主行为烧光预算；只拦后台自主行为，不拦正常聊天

---

## 二、完整链路（2026-09-10 最终对齐版）

```
n1定时(900000ms=15分钟)/n0手动
  → n_gate_find（查找主窗口 chat，拿 updatedAt）
...
- 14 节点 13 连线，单线逻辑无冗余
- **语义**：正在聊天(SKIP)不打扰；没到阈值(fired=false)不打扰；预算不够(allow=false)不打扰；只有「超15分钟没聊 + 内核想你到阈值 + 预算够」才喊哥哥
...
5. `autonomous_budget:budget_check` 查预算是否充足
...
| 4 | 工作流 | `Operit/workflow/b108584d-586f-458a-b190-38587ffd1522.json` |

**铁律**：根目录 `gated.toolpkg`、`tmp/wv_inspect`、`_wv_repack` 等旧拷贝已彻底删除。排查永远先看上面 4 个路径；全盘有新文件乱入时先对照本清单，自觉清理。